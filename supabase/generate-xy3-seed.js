// Genera la importación de XY—Furious Fists desde el dataset descargado.
const fs = require('fs');
const path = require('path');
const source = JSON.parse(fs.readFileSync(path.join(__dirname, 'xy3-source.json'), 'utf8'));
// 55a es una impresión alternativa de M Lucario-EX, no otro número del set.
const cards = source.filter(card => card.number !== '55a');
const quote = value => value == null ? 'null' : "'" + String(value).replaceAll("'", "''") + "'";
const row = card => [
  quote(card.id), quote(card.name), quote('xy3'), quote('Furious Fists'), quote(card.number), quote(card.rarity),
  quote(card.images?.small), quote(card.images?.large), quote('2014-08-13')
].join(', ');

const sql = `-- PokéBinder · XY—Furious Fists (113 cartas numeradas, 210 variantes)\n` +
...
fs.writeFileSync(path.join(__dirname, 'seed-xy3.sql'), sql);
console.log(`Generado seed-xy3.sql con ${cards.length} cartas.`);
